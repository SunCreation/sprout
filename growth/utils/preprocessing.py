...
def remove_outlier(df: pd.DataFrame, out_df:pd.DataFrame = None):
...
def dummy_onehot_concat(df: pd.DataFrame, cols: list):
    concat_target = [df]
    for col in cols:
        concat_target.append(pd.get_dummies(df[col]))
    
    df = pd.concat(concat_target,axis=1)
    return df
